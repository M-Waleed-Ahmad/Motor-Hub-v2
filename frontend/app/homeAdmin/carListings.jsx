...
export default function ProductListingsPage() {
  ...
  const [vehicleTypeFilter, setVehicleTypeFilter] = useState('all');
  const [modalVisible, setModalVisible] = useState(false);
  const [noVehiclesModalVisible, setNoVehiclesModalVisible] = useState(false);
  const router = useRouter();
  ...
  const fetchVehicles = useCallback(async () => {
    try {
      ...
      if (data.length === 0) {
        setNoVehiclesModalVisible(true); // Show the modal if no vehicles are found
        setVehicles([]);
        setFilteredVehicles([]);
        return;
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <Modal
        animationType="slide"
        transparent
        visible={noVehiclesModalVisible}
        onRequestClose={() => setNoVehiclesModalVisible(false)}
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>No Vehicles Available</Text>
            <Text style={styles.modalText}>
              Currently, there are no vehicles listed for {activeTab.toLowerCase()}.
            </Text>
            <TouchableOpacity
              onPress={() => setNoVehiclesModalVisible(false)}
...
const styles = StyleSheet.create({
  ...
  modalText: { color: '#aaa', fontSize: 14, marginBottom: 20, textAlign: 'center' },
  closeModalButton: { backgroundColor: '#00b4d8', padding: 10, borderRadius: 5 },
  closeModalButtonText: { color: '#fff', textAlign: 'center', fontSize: 16 },
});
