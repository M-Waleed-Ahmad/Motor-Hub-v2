...
export default function ProductListingsPage() {
  const [activeTab, setActiveTab] = useState('Listings Owned');
  ...
  const fetchVehicles = useCallback(async () => {
    try {
      ...
      const response = await fetch(`http://192.168.18.225:8000/sales?user_id=${userId}`);
      if (!response.ok) {
  ...
  const filterVehicles = useCallback(
    (vehicles, tab, availability, type, query) => {
      ...
      if (tab === 'Listings Owned') {
        filtered = filtered.filter((vehicle) => vehicle.user_id === 1); // Replace with actual user ID
      } else if (tab === 'Listings Bought') {
        filtered = filtered.filter((vehicle) => vehicle.user_id !== 1); // Replace with actual user ID
      }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabs}>
        <TouchableOpacity
          style={[styles.tab, activeTab === 'Listings Owned' && styles.activeTab]}
          onPress={() => handleTabChange('Listings Owned')}
        >
          <Text style={[styles.tabText, activeTab === 'Listings Owned' && styles.activeTabText]}>Listings Owned</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.tab, activeTab === 'Listings Bought' && styles.activeTab]}
          onPress={() => handleTabChange('Listings Bought')}
        >
          <Text style={[styles.tabText, activeTab === 'Listings Bought' && styles.activeTabText]}>Listings Bought</Text>
        </TouchableOpacity>
      </View>
